...
async function loadData() {
  loading.value = true
  try {
    // NOTA: `GET /departments` e `GET /workflows` ainda não existem
    // na API — esta tela já está pronta pra consumi-los assim que
    // existirem (o modelo de dados de Department já está no banco;
    // Workflow/regras de bot ainda precisam ser modelados).
    ...
    if (workflowResult.status === "fulfilled" && workflowResult.value) {
      Object.assign(workflow, workflowResult.value)
...
async function saveWorkflow() {
  ...
  try {
    ...
    saveError.value = err?.data?.message ?? "A API de fluxos ainda não está disponível neste ambiente."
  } finally {
    saving.value = false
  }
}
